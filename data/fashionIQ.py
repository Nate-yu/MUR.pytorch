...
from data.utils import get_img_from_path
from data.abc import AbstractBaseDataset

_DEFAULT_FASHION_IQ_DATASET_ROOT = 'D:/Datasets/fashionIQ'
_DEFAULT_FASHION_IQ_VOCAB_PATH = 'D:/Datasets/fashionIQ/fashion_iq_vocab.pkl'

''' 从指定的数据集根目录、服装类型和数据集分割加载JSON格式的图像标题数据 '''
...
''' 从指定的数据集根目录、服装类型和数据集分割加载文本格式的图像标题数据 '''
def _get_img_caption_txt(dataset_root, clothing_type, split):
    with open(
            os.path.join(dataset_root, 'captions_pairs', 'fashion_iq-{}-cap-{}.txt'.format(split, clothing_type))) as f:
        file_content = f.readlines()
    return file_content


''' 从指定的数据集根目录、服装类型和数据集分割加载图像分割数据 '''
...
''' 根据索引从图像标题数据中获取修改器（modifier），用于描述图像之间的变化 '''
...
''' 用于加载和处理FashionIQ数据集的测试数据，仅返回目标样本和其唯一ID '''
class FashionIQDataset(AbstractBaseFashionIQDataset):
    ...
    def __getitem__(self, idx):
        ...
        reference_img = get_img_from_path(ref_img_path, self.img_transform)
        target_img = get_img_from_path(targ_img_path, self.img_transform)
...
''' 用于加载和处理FashionIQ数据集的查询数据，返回参考样本、修改器、目标属性和修改器长度 '''
class FashionIQTestDataset(AbstractBaseFashionIQDataset):
    ...
    def __getitem__(self, idx, use_transform=True):
        ...
        target_img = get_img_from_path(img_path, img_transform)
...
class FashionIQTestQueryDataset(AbstractBaseFashionIQDataset):
    ...
    def __getitem__(self, idx, use_transform=True):
        ...
        ref_img_path = os.path.join(self.img_root_path, self.ref_img_path[idx])
        ref_img = get_img_from_path(ref_img_path, self.img_transform)
        ref_id = self.ref_img_path[idx].split('/')[-1].split('.')[0]
        targ_id = self.targ_img_path[idx].split('/')[-1].split('.')[0]
