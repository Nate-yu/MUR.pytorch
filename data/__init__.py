...
from data.collate_fns import PaddingCollateFunction, PaddingCollateFunctionTest, BertPaddingCollateFunction, \
    BertPaddingCollateFunctionTest
...
def create_dataloaders(image_transform, text_transform, configs):
    ...
    train_val_dataloaders = {key: test_dataloader_factory(datasets=value, config=configs, collate_fn=collate_fn_test)
                             for key, value in train_val_datasets.items()}
