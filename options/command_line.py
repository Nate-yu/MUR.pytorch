...
# Load Template
#########################
parser.add_argument('--config_path', type=str, default='configs/shoes_config.json', help='config json path')
...
parser.add_argument('--epoch', type=int, default=1, help='epoch (default: 80)')
...
#########################
parser.add_argument('--dataset', type=str, default='shoes', help='Dataset')
...
parser.add_argument('--image_encoder', type=str, default='modified_resnet', help='Image Model')
...
parser.add_argument('--text_encoder', type=str, default="roberta", help='Text Model')
...
parser.add_argument('--wandb_account_name', type=str, default='yubin06', help='Weights & Biases account name')

#########################
# Resume Training
#########################
parser.add_argument('--checkpoint_path', type=str, default='./ckpt', help='Path to saved checkpoint file')
...
parser.add_argument('--experiment_description', type=str, default='test_fashionIQ', help='Experiment description')
...
def load_config_from_command():
    # 首先通过 _get_user_defined_arguments() 方法获取用户定义的参数列表
    user_defined_argument = _get_user_defined_arguments(sys.argv[1:])

    # 使用 parser.parse_args() 解析命令行参数，并根据用户定义的参数列表筛选出相关的配置，存储在 user_defined_configs 字典中
    configs = vars(parser.parse_args())
    user_defined_configs = {k: v for k, v in configs.items() if k in user_defined_argument}

    # 返回两个字典：configs（包含所有命令行参数）和 user_defined_configs（只包含用户指定的参数）
